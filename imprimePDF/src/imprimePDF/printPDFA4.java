	...
	public static void imp_factura(String _file_xml, factura_cabecera Cabecera, factura_detalle[] Detalle, int _lineas_de_la_factura, String _file_pdf, String _file_jpg) throws DocumentException, IOException {
	 ...
	 // 	     Paragraph _linea01 = new Paragraph();
	  ...
	  //	    Cabecera.set_total(0);
	  	   ...
	  	   boolean _servicio = false;
	  	   
	  	   
	  	   
	  	 String palabra = "servicio";
	  	 String palabra2 = "procesamiento";
	  	String palabra3 = "arrendamiento";
	  	String palabra4 = "alta";
	  	...
	  	//    	 Cabecera.set_total(Cabecera.get_subtotal()+Cabecera.get_total_igv());
	  	    		 ...
	  	    		 if (_servicio==false) {
	  	    			 String texto = Detalle[i].get_descripcion();
		  	    		 _servicio = texto.contains(palabra);
	  	    		 }


	  	    		 if (_servicio==false) {
	  	    			 String texto = Detalle[i].get_descripcion();
	 	  	    		 _servicio = texto.contains(palabra2);
	  	    		 }


	  	    		 if (_servicio==false) {
	  	    			 String texto = Detalle[i].get_descripcion();
	 	  	    		 _servicio = texto.contains(palabra3);
	  	    		 }
	  	    		 
	  	    		 if (_servicio==false) {
	  	    			 String texto = Detalle[i].get_descripcion();
	 	  	    		 _servicio = texto.contains(palabra4);
	  	    		 }
		        ...
		        if ((Cabecera.get_total()>=_tope) && (_servicio==true)) {
		    	      // leyenda
			        canvas.saveState();                               // q
			        canvas.beginText();                               // BT
		     ...
		     // canvas.showText("Total:      "+Cabecera.get_total());	        // (Hello World)Tj
        	    ...
        	    if (Cabecera.get_moneda().equals("USD")) {
     	        	...
     	        	canvas.showText("s/ "+Formato.dinero(Cabecera.get_total_gratuitas()));
     	        }
		        ...
		        canvas.saveState();                               // q
		        canvas.beginText();                               // BT
		        canvas.moveText(497, 68);                         // 36 788 Td
		        canvas.setFontAndSize(bf, 8); // /F1 12 Tf
		     // canvas.showText("Total:      "+Cabecera.get_total());	        // (Hello World)Tj
